...
function GroupCard({
    ...
    const [activeSubgroupKey, setActiveSubgroupKey] = useState<string | null>(
        null,
    );
    ...
    const [subgroupQuestionsMap, setSubgroupQuestionsMap] = useState<{
        [key: string]: Question[];
    }>({});
    ...
    const handleAnswerChange = (
        ...
        setAnswers((prev) =>
            prev.map((a, idx) =>
                idx === i
                    ? { ...a, [field]: field === "score" ? Number(value) : value }
                    : a,
            ),
        );
    };
    ...
    const handleUpdateQuestion = (
        ...
        setSubgroupQuestionsMap((prev) => ({
            ...prev,
            [categoryKey]: (prev[categoryKey] || []).map((q, i) =>
                i === questionIndex ? updatedQuestion : q
            ),
        }));
    };

    const handleDeleteQuestion = (subgroupKey: string, questionIndex: number
    ) => {
        setSubgroupQuestionsMap(prev => ({
            ...prev,
            [subgroupKey]:
                (prev[subgroupKey] || []).filter(
                    (_, i) => i !== questionIndex
                ),
        }));
    };

    const [subgroups, setSubgroups] = useState<{ text: string }[]>([]);
                                    ...
                                    <div className="flex items-center mb-3">
                                        <div className="flex-1">
                                            <p className="font-semibold text-lg">
                                                {sub.text || `Subgroup ${subIndex + 1}`}
                                            </p>
                                        </div>

                                        <div className="flex-1 items-center gap-4">
                                            <p className="font-semibold text-lg">
                                                Questions ({subgroupQuestionCount})
                                            </p>
                                        </div>
                                        ...
                                        <Button
                                            onClick={() => {
                                                const subgroupKey = `${index}-${subIndex}`;
                                                setActiveSubgroupKey(subgroupKey);
                                                setShowCategoryModal(true);
                                                setTimeout(() => setAnimateCategoryModal(true), 10);
                                            }}
                                                ...
                                                >
                                                    ...
                                                    <div className="flex items-center justify-between">
                                                        ...
                                                        <div className="flex items-center gap-2">
                                                            <Button
                                                                onClick={() => {
                                                                    setActiveCategoryKey(`${index}-${subIndex}-${categoryIndex}`);

                                                                    setShowQuestionModal(true);

                                                                    setTimeout(() => {
                                                                        setAnimateQuestionModal(true);
                                                                    }, 10);
                                                                }}
                                                            ...
                                                            <Button
                                                                onClick={() => {
                                                                    const categoryKey = `${index}-${subIndex}-${categoryIndex}`;
                            ...
                            {subgroups.map((subgroup, i) => (
                                <div key={i} className="border rounded-xl p-3">
                                    ...
                                    <div className="flex items-center gap-3">
                                        <input
                                            value={subgroup.text}
                                            onChange={(e) => handleSubgroupChange(i, e.target.value)}
                                    ...
                                    <div className="flex items-end gap-3">
                                        ...
                                        <div className="flex flex-col w-24">
                                            ...
                                            <input
                                                ...
                                                px-2 py-2 text-center bg-gray-50"
                                            />
                                        </div>
