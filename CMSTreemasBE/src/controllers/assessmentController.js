...
const getAssessmentScore = async (req, res) => {
    try {
        ...
        // 1. Tarik Nama Perusahaan & Result ID
        const query = `
            SELECT 
                b.nama_umkm AS company_name,
                r.id AS result_id
        ...
        // 2. Jika sudah ada result_id, lakukan perhitungan hanya untuk 3 Pilar
        if (profileData.result_id) {
            ...
            rows.forEach(r => {
                ...
                if (!structure[secName]) {
                    ...
                    structure[secName] = { pillar: pillar, subgroups: {} };
                }
                
                if (!structure[secName].subgroups[subName]) structure[secName].subgroups[subName] = { categories: {} };
            ...
            // Hitung rata-rata akhir per pilar
            let strategicScore = avg(strategicSubScores); 
            let financialScore = avg(financialSubScores); 
            let coreScore = avg(coreSubScores);           
        ...
        console.error('Error getAssessmentScore:', error);
        res.status(500).json({ success: false, error: 'Gagal mengambil skor penilaian.' });
    }
};
...
module.exports = {
    ...
    addOption, updateOption, deleteOption, getProfileDetail, getUserAnswers, saveInsight, getAssessmentScore
};