...
// X axis is always 1–4
const X_MIN = 1, X_MAX = 4;
const X_THRESHOLD = 2.5;
...
const quadrantColors = [
  '#d9ead3', // III top-left
  '#d9ead3', // IV top-right
  '#f4cccc', // I bottom-left
  '#fff2cc', // II bottom-right
];

export default function QuadrantChart({ coreDrivers, financialHealth, strategicValue, financialMax = 100 }: Props) {
  const isMax4 = financialMax === 4;

  const Y_MIN = isMax4 ? 1 : 40;
  const Y_MAX = isMax4 ? 4 : 100;
  const Y_THRESHOLD = isMax4 ? 2.5 : 70;
  const yTicks = isMax4 ? [1.0, 1.75, 2.5, 3.25, 4.0] : [40, 55, 70, 85, 100];
  ...
  // X-axis labels
  const xTicks = [1.0, 2.5, 4.0];

  return (
    <svg width={W} height={H} style={{ fontFamily: 'Arial, sans-serif' }}>
      {/* Quadrant backgrounds */}
      {/* III top-left */}
      <rect x={PADDING.left} y={PADDING.top} width={thresholdX - PADDING.left} height={thresholdY - PADDING.top} fill={quadrantColors[0]} />
      {/* IV top-right */}
      <rect x={thresholdX} y={PADDING.top} width={PADDING.left + PLOT_W - thresholdX} height={thresholdY - PADDING.top} fill={quadrantColors[1]} />
      {/* I bottom-left */}
      <rect x={PADDING.left} y={thresholdY} width={thresholdX - PADDING.left} height={PADDING.top + PLOT_H - thresholdY} fill={quadrantColors[2]} />
      {/* II bottom-right */}
      <rect x={thresholdX} y={thresholdY} width={PADDING.left + PLOT_W - thresholdX} height={PADDING.top + PLOT_H - thresholdY} fill={quadrantColors[3]} />
      ...
      {/* Quadrant labels */}
      {quadrants.map((q) => (
        <text key={q.label} x={q.x} y={q.y} fontSize={9} fill="#555">{q.label}</text>
      ))}
      ...
      {xTicks.map((v) => {
        ...
        return (
          <g key={v}>
            ...
            <text x={x} y={PADDING.top + PLOT_H + 13} fontSize={9} textAnchor="middle" fill="#555">{v.toFixed(2)}</text>
          </g>
        );
      })}
      ...
      {/* Data point */}
      <circle cx={px} cy={py} r={12} fill="#4472c4" fillOpacity={0.85} />
      <text x={px} y={py + 4} fontSize={10} textAnchor="middle" fill="white" fontWeight="bold">
        {strategicValue.toFixed(2)}
      </text>
    </svg>
  );
}
