...
function AssesmentGroup() {
    ...
    const handleAddGroup = async (group: Group) => {
        try {
            const res = await fetch("/api/assessment/groups", {
            ...
            if (group.subgroups.length > 0) {
                await Promise.all(group.subgroups.map((sg, idx) => 
                    fetch("/api/assessment/subgroups", {
                        method: "POST", headers: { "Content-Type": "application/json", "Authorization": `Bearer ${token}` },
    ...
    const handleEditGroup = async (index: number, updatedGroup: Group) => {
        ...
        if(groupId) {
            await fetch(`/api/assessment/groups/${groupId}`, {
            ...
            for (let i = 0; i < updatedGroup.subgroups.length; i++) {
                ...
                if (sg.id) {
                     await fetch(`/api/assessment/subgroups/${sg.id}`, {
                        method: "PUT", headers: { "Content-Type": "application/json", "Authorization": `Bearer ${token}` },
                        body: JSON.stringify({ name: sg.text })
                    });
                } else {
                     await fetch(`/api/assessment/subgroups`, {
                        method: "POST", headers: { "Content-Type": "application/json", "Authorization": `Bearer ${token}` },
