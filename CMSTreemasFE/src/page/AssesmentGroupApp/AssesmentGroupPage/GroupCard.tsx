...
const handleUpdateQuestion = async (_categoryKey: string, _questionIndex: number, updatedQuestion: Question) => {
        const questionId = updatedQuestion.id;
        if(!questionId) return;
        const token = localStorage.getItem("token") || "";

        // 1. Update teks pertanyaan
        await fetch(`/api/assessment/questions/${questionId}`, {
    ...
    const handleDeleteQuestion = async (categoryKey: string, questionIndex: number) => {
        ...
        if(qId) {
            await fetch(`/api/assessment/questions/${qId}`, {
                                                    ...
                                                    <div className="flex items-center justify-between">
                                                        ...
                                                        <div className="flex items-center gap-2">
                                                            ...
                                                            <Button onClick={() => {
                                                                    ...
                                                                    if(catId) {
                                                                        fetch(`/api/assessment/categories/${catId}`, { method: "DELETE", headers: { "Authorization": `Bearer ${localStorage.getItem("token")}` } }).then(() => { if(onRefresh) onRefresh(); });
                                                                    }
                                                                }}
