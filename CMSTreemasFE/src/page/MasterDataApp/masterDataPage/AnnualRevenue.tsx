...
function AnnualRevenue (){
    ...
    const fetchAnnualRevenues = async () => {
        try {
            const response = await fetch("/api/master-data/revenue_ranges");
    ...
    const handleSave = async () => {
        ...
        try {
            if (editingId) {
                await fetch(`/api/master-data/revenue_ranges/${editingId}`, {
                    method: "PUT",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify(payload),
                });
            } else {
                await fetch("/api/master-data/revenue_ranges", {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify(payload),
                });
    ...
    const handleDelete = async (id: number) => {
        ...
        try {
            await fetch(`/api/master-data/revenue_ranges/${id}`, { method: "DELETE" });
            fetchAnnualRevenues();
        } catch (error) {
