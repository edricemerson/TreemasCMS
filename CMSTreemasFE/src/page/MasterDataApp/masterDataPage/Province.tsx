...
function Province() {
    ...
    const fetchProvinces = async() => {
        try {
            const response = await fetch("/api/master-data/provinces");
    ...
    const handleSave = async () => {
        ...
        try {
            if(editingId) {
                await fetch(`/api/master-data/provinces/${editingId}`, {
                    method: "PUT",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify(payload),
                });
            }else {
                await fetch("/api/master-data/provinces", {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify(payload),
                });
    ...
    const handleDelete = async (id: number) => {
        ...
        try {
            await fetch(`/api/master-data/provinces/${id}`, {
                method: "DELETE",
            });
            fetchProvinces();
        }catch(error){
