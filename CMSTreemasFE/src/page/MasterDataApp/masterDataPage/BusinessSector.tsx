...
function BusinessSector (){
    ...
    const fetchBusinessSectors = async () => {
        try {
            const response = await fetch("/api/master-data/business_sectors");
    ...
    const handleSave = async () => {
        ...
        try {
            if (editingId) {
                await fetch(`/api/master-data/business_sectors/${editingId}`, {
                    method: "PUT",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify(payload),
                });
            } else {
                await fetch("/api/master-data/business_sectors", {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify(payload),
                });
    ...
    const handleDelete = async (id: number) => {
        ...
        try {
            await fetch(`/api/master-data/business_sectors/${id}`, {
                method: "DELETE",
            });
            fetchBusinessSectors();
        } catch (error) {
