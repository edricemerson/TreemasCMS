...
function CompanySize (){
   ...
   const fetchCompanySizes = async () => {
        try {
            const response = await fetch("/api/master-data/company_sizes");
   ...
   const handleSave = async () => {
        ...
        try {
            if (editingId) {
                await fetch(`/api/master-data/company_sizes/${editingId}`, {
                    method: "PUT",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify(payload),
                });
            } else {
                await fetch("/api/master-data/company_sizes", {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify(payload),
                });
   ...
   const handleDelete = async (id: number) => {
        ...
        try {
            await fetch(`/api/master-data/company_sizes/${id}`, { method: "DELETE" });
            fetchCompanySizes();
        } catch (error) {
