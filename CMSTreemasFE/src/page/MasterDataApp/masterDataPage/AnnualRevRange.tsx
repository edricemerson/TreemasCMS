...
function AnnualRevRange (){
    ...
    const fetchCompanySizes = async () => {
        try {
            const response = await fetch("/api/master-data/annual-revenue");
   ...
   const handleSave = async () => {
        ...
        try {
            if (editingId) {
                await fetch(`/api/master-data/annual-revenue/${editingId}`, {
                    method: "PUT",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify(payload),
                });
            } else {
                await fetch("/api/master-data/annual-revenue", {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify(payload),
                });
  ...
  const handleDelete = async (id: number) => {
        ...
        try {
            await fetch(`/api/master-data/annual-revenue/${id}`, { method: "DELETE" });
            fetchCompanySizes();
        } catch (error) {
