...
function IndustryType (){
    ...
    const fetchIndustryTypes = async () => {
        try {
            const response = await fetch("/api/master-data/industries");
    ...
    const handleSave = async () => {
        ...
        try {
            if (editingId) {
                await fetch(`/api/master-data/industries/${editingId}`, {
                    method: "PUT",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify(payload),
                });
            } else {
                await fetch("/api/master-data/industries", {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify(payload),
                });
    ...
    const handleDelete = async (id: number) => {
        ...
        try {
            await fetch(`/api/master-data/industries/${id}`, { method: "DELETE" });
            fetchIndustryTypes();
        } catch (error) {
